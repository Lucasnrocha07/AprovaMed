...
const empty = () => ({ nome: "", conteudos: "", datas: "", estrategias: "", observacoes: "" });
...
export default function Vestibulares() {
  ...
  const save = async () => {
    try {
      if (editing) await api.put(`/vestibulares/${editing}`, form);
      else await api.post("/vestibulares", form);
    ...
    <div className="space-y-5" data-testid="vestibulares-root">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="font-heading text-4xl font-black mt-1">Anotações livres</h1>
        </div>
            ...
            return (
              <li key={v.id} className={`${tones[idx % tones.length]} nb-border nb-shadow rounded-2xl p-5`} data-testid={`vest-${v.id}`}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="font-heading text-xl font-black text-zinc-900">{v.nome}</h3>
                </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="nb-border max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <div>
              <Label className="font-bold">Datas</Label>
